...
def startGame():
    ...
    cam = pygame.camera.Camera(device[0], [320, 320])
    #for _ in range(1):
        #time.sleep(1)
    cam.start()
    # 游戏主循环
    ...
    while running:
        if enemy_frequency%camShuffleValue == 0:
            ...
            if (next_loc_x ==0 and next_loc_y==0) or (x_==0 and y_==0): # 第一次检测或者检测不到
        ...
        thresholdValue = 5 # 提升操控性
        # 处理键盘事件（移动飞机的位置）

        if key_pressed[K_w] or key_pressed[K_UP] or Ver<-1*thresholdValue :
            player.moveUp(np.clip(np.sqrt(np.abs(Ver)),0,5))
        ...
        for enemy in enemies1:
            ...
            if pygame.sprite.collide_circle(enemy, player):
                ...
                break
            # 移动出屏幕后删除飞机，游戏结束
            if enemy.rect.top > SCREEN_HEIGHT-50:# 源码有误修改
                enemies1.remove(enemy) 
                player.is_hit = True
                break
        # 敌机被子弹击中效果处理
